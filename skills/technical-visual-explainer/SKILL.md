...
Color the meaningful internal icons/elements by function, not only their surrounding cards. When colored icons are requested, retain recognizable filled icons for key concepts even in a precise scientific workflow; density alone is not permission to replace them with colored boxes. Allocate space or separate overview and detail. Use a legend only when labels, icons and local keys leave a necessary encoding unclear; otherwise omit it. When needed, keep it small, readable and visually quiet. Preserve color semantics even when no legend is shown. Stage identity, component role, current focus and implementation status are separate encodings. Every color, fill, fade, gradient, and emphasis must encode a recorded meaning; omit effects that carry no information. When extending an existing harness, preserve that foundation relationship explicitly rather than implying a second independent execution system.

Keep both overview and detail diagrams label-led: short names, inputs/outputs, checks and branch conditions. Detail means more precise relationships, not explanatory sentences. Put takeaways, rationale and spoken scope explanations in notes or an external caption; retain only the shortest in-image status needed to avoid a false claim. Apply the text and legend checks in [visual-grammar](references/visual-grammar.md) to the actual rendered image, not only the prompt.
