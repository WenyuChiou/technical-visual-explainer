...
- Keep a compact visible legend within any image that needs category decoding; detached README or slide images must not depend on a previous page or spoken explanation. Repeat the same legend order, labels, icons and colors across the family. A decorative accent palette that encodes no classes need not become a taxonomy legend.
- Make the legend visually subordinate: short labels, small color/icon samples, no competing heading or large enclosing card. For a landscape overview, a slim footer around 5–7% of canvas height is a useful starting point, not a universal quota. If it needs more space, shorten labels or use nearby keys; do not shrink it below readability. Check the actual legend-to-main-diagram proportion at the delivery size. A feedback mapping is substantive model content, not a legend to squeeze into that allowance.
...
Use short verb/object module labels and noun output labels. A heading describes the subject; a second subtitle is needed only when it changes interpretation. Prefer one concise scope statement over repeated status badges on every box. Explanation belongs in cues, not long in-image paragraphs.
