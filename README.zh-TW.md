...
完整解說圖預設使用環境內建的產圖工具。你也可以保留生成主圖，
額外要求**可編輯副本**：生成後重建或轉換文字、模組與連線，
生成的 icon 則可能保留為可移動的點陣圖片。Agent 必須實測宣告的修改功能，
並說明外觀差異，不能宣稱還原了原始圖層。若明確要求只用原生圖形，
會以你的要求為準。必要工具不可用時，必須說明尚未完成的部分。

圖例應小而可讀；要求彩色 icon 時，精密的科學流程圖也應在關鍵概念上保留
有意義的實色圖示。關係正確、檔案可編輯，仍不等於設計符合要求。
